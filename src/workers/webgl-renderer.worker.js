// WebGL Renderer Web Worker
// Handles all WebGL rendering using OffscreenCanvas

class WebGLRendererWorker {
    constructor(canvas) {
        this.canvas = canvas;
    ...
    render(videoFrame1, videoFrame2, progress, transitionType) {
        const gl = this.gl;
        ...
        if (videoFrame1) {
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, videoFrame1);
        }

        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, this.texture2);
        if (videoFrame2) {
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, videoFrame2);
        }
    ...
    try {
        switch (type) {
            case 'init':
                ...
                renderer = new WebGLRendererWorker(canvas);
                self.postMessage({ type: 'init-complete' });
                break;

            case 'render':
                const { videoFrame1, videoFrame2, progress, transitionType } = data;
                if (renderer) {
                    if (videoFrame2) {
                        renderer.render(videoFrame1, videoFrame2, progress, transitionType);
                    } else {
                        renderer.renderSingleFrame(videoFrame1);
                    }
                }

                // Close ImageBitmaps to free memory after rendering
                if (videoFrame1 && typeof videoFrame1.close === 'function') {
                    videoFrame1.close();
                }
                if (videoFrame2 && typeof videoFrame2.close === 'function') {
                    videoFrame2.close();
                }
